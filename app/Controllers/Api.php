...
class Api extends ResourceController
{
    ...
    public function polyline() 
    {
        ...
        $query = $db->query("SELECT id, ST_AsGeoJSON(geom) AS geom, 
                ...
                nama, deskripsi, foto, created_at, updated_at FROM tbl_data_polyline");
        $query_array = $query->getResultArray();
        $feature = array();
    ...
    public function polygon() 
    {
        $db = db_connect();
        $query = $db->query("SELECT id, ST_AsGeoJSON(geom) AS geom, 
                ST_Area(ST_Transform(ST_SetSRID(ST_AsText(geom),4326),32748)) AS luas,
                nama, deskripsi, foto, created_at, updated_at FROM tbl_data_polygon");
        $query_array = $query->getResultArray();
        $feature = array();
